...
6. ⭐⭐ CRITICAL 题目提取要求（最高优先级）：
   ...
   · 任何带答案的题目
7. 最小题目数保证：
   - 如果文本中有超过 10 页内容，必须至少提取 5-10 道题目
   - 如果有 5-10 页内容，必须至少提取 3-5 道题目
   - 优先填充数量，即使题目不完整也要尽力提取
8. 如果找不到多选项，可以用简答形式提取（但必须有题干和答案）
9. 题目完整性检查：
   - 保证题干内容完整（不截断）
   - 如有选项则完整提取所有选项（A、B、C、D 都要）
   - 必须提供答案（即使不确定也要给出最可能的答案）
   - 尽可能提供解析说明
10. 只输出一个完整的 JSON 对象，不要输出多个 JSON
11. ⭐⭐ 最终检查：
    提交前请检查：
    - 题目总数是否满足最小要求？
    - 是否遗漏了任何包含"答案""解析""选项"的内容？
    - 每个知识点是否都有至少 1-2 道题目关联？
    如果不满足，继续补充题目直到满足为止"""

        prompt = f"""请从以下公务员考试"{category}"相关文本中按知识点提取题目和知识点：
...
【最高优先级】提取策略：
1. 第一步：仔细阅读全文，标记所有题目所在位置
2. 第二步：识别文本中的核心知识点
3. 第三步：找出与每个知识点相关的题目
4. 第四步：建立知识点和题目的关联关系
5. 第五步：为每道题目标注相关的知识点
6. 第六步：为每个知识点列出相关的题目 ID
7. 最小保证：
   · 每个知识点都至少有 1-2 道相关题目
   · 每道题目都与 1-3 个知识点关联
   · 题目总数不少于 5-10 道（取决于文本长度）
8. 格式要求：
   · 严格按照 JSON 格式输出
   · 确保只输出一个完整的 JSON 对象
   · 不要在 JSON 前后添加其他文本
...
- 这是题目提取任务中最关键的步骤
- 宁可多提取也不要遗漏题目
- 题目完整性比数量质量更重要（至少先保证有足够的题目）
- 如果不确定是否是题目，只要有题干和答案就要提取"""
